...
// New function adapted to meassure 25V and 5A
//by: Nicolas Orcasitas Garcia and Jorge Eduardo Angarita Perez
void Adafruit_INA219::setCalibration_32V_5A() {
  ...
  // VBUS_MAX = 25V
  // VSHUNT_MAX = 0.04          (Assumes Gain 1, 40mV)
  // RSHUNT = 0.007               (Resistor value in ohms)
  ...
  // 3. Calculate possible range of LSBs (Min = 15-bit, Max = 12-bit)
  // MinimumLSB = MaxExpected_I/32767
  // MinimumLSB = 0.00015259              (153uA per bit)
  // MaximumLSB = MaxExpected_I/4096
  // MaximumLSB = 0.0012207              (1220uA per bit)

  // 4. Choose an LSB between the min and max values
  //    (Preferrably a roundish number close to MinLSB)
  // CurrentLSB = 0.000153 (153uA per bit)

  // 5. Compute the calibration register
  // Cal = trunc (0.04096 / (Current_LSB * RSHUNT))
  // Cal = 38244 (0x2000)

  ina219_calValue = 35115;

  // 6. Calculate the power LSB
  // PowerLSB = 20 * CurrentLSB
  // PowerLSB = 0.00306 (1mW per bit)

  // 7. Compute the maximum current and shunt voltage values before overflow
  //
  // Max_Current = Current_LSB * 32767
  // Max_Current = 5.01335A before overflow
  //
  // If Max_Current > Max_Possible_I then
  //    Max_Current_Before_Overflow = MaxPossible_I
  // Else
  ...
  //
  // Max_Current_Before_Overflow = MaxPossible_I
  // Max_Current_Before_Overflow = 5.01335
  //
  // Max_ShuntVoltage = Max_Current_Before_Overflow * RSHUNT
  // Max_ShuntVoltage = 0.0350935V
  //
  // If Max_ShuntVoltage >= VSHUNT_MAX
  //    Max_ShuntVoltage_Before_Overflow = VSHUNT_MAX
  // Else
  ...
  //
  // Max_ShuntVoltage_Before_Overflow = VSHUNT_MAX
  // Max_ShuntVoltage_Before_Overflow = 0.0350935V

  // 8. Compute the Maximum Power
  // MaximumPower = Max_Current_Before_Overflow * VBUS_MAX
  // MaximumPower = 5 * 32V
  // MaximumPower = 160.4272W

  // Set multipliers to convert raw current/power values
  ina219_currentDivider_mA = 6;    // Current LSB = 153uA per bit (1000/153 = 6.54)
  ina219_powerMultiplier_mW = 1.0f; // Power LSB = 1mW per bit
